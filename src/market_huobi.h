...
private:
  ...
  void OnGetMarketDepthWebsocket(std::shared_ptr<AsioHttpsRequest> request, std::shared_ptr<HttpResponseMsgStruct> responce, std::shared_ptr<AsioHttpsSocket> socket);
public:
...
private:
  ...
  bool HandleWebSocketMarketDepth(boost::property_tree::ptree& pt, std::string& str_out);
  void InitMarketPair();
private:
  void HandleAMessage(std::shared_ptr<AsioHttpsSocket> socket, std::shared_ptr<WebSocketMessage> msg);
private:
  bool b_trade_history_connected;
  std::shared_ptr<AsioHttpsSocket> websocket_history_transaction_;
  std::string trade_history_websocket_receive_;
  bool b_market_depth_connected;
  std::shared_ptr<AsioHttpsSocket> websocket_market_transaction_;
  std::string market_depth_websocket_receive_;
