...
void MarketHuobi::StartWatch(){
  ...
  StartGetTradeHistory();
  StartGetMarketDepth();
}
void MarketHuobi::StartGetTradeHistory(){
  b_trade_history_connected = false;
  ...
  websocket_history_transaction_->Process(request, boost::bind(&MarketHuobi::OnGetTradeHistoryWebsocket, this, _1,_2, websocket_history_transaction_));
}

void MarketHuobi::StartGetMarketDepth(){
  b_market_depth_connected = false;
  std::shared_ptr<AsioHttpsRequest> request = std::make_shared<AsioHttpsRequest>();
  request->config_.ssl_ = true;
  request->config_.use_proxy_ = true;
  request->config_.websocket_ = true;
  ...
  request->head_.SetAttribute("Sec-WebSocket-Version", "13");
  request->head_.url_ = "/ws";
  websocket_market_transaction_->Process(request, boost::bind(&MarketHuobi::OnGetMarketDepthWebsocket, this, _1,_2, websocket_market_transaction_));
}
...
void MarketHuobi::OnGetTradeHistoryWebsocket(std::shared_ptr<AsioHttpsRequest> request, std::shared_ptr<HttpResponseMsgStruct> responce, std::shared_ptr<AsioHttpsSocket> socket)
{
  if(responce->error_ == ""){
    ...
    std::shared_ptr<WebSocketMessage> msg = std::make_shared<WebSocketMessage>();
    if(!b_trade_history_connected){
      //链接成功后，发送订阅信息
      for(std::pair<std::string, std::vector<std::string>> item1 : market_pair_map_){
        for(std::string item2:item1.second){
          //std::string cmd = "{\"sub\": \"market.btcusdt.trade.detail\",\"id\": \"id generated by client\"}";
          std::string cmd = (boost::format("{\"sub\": \"market.%2%%1%.trade.detail\",\"id\": \"id generated by client\"}")
                             %boost::algorithm::to_lower_copy((item1.first=="USD")?"USDT":item1.first)
                             %boost::algorithm::to_lower_copy(item2)).str();
          std::string send_msg = WebSocketMessage::Encode(cmd);
          socket->SocketSend(send_msg);
          b_trade_history_connected = true;
        }
      }
    }
    while(msg->Decode(trade_history_websocket_receive_)){
      boost::asio::post(thread_pool_, boost::bind(&MarketHuobi::HandleAMessage, this, socket, msg));
    }
  }else{
...
void MarketHuobi::OnGetMarketDepthWebsocket(std::shared_ptr<AsioHttpsRequest> request, std::shared_ptr<HttpResponseMsgStruct> responce, std::shared_ptr<AsioHttpsSocket> socket)
{
  if(responce->error_ == ""){
    market_depth_websocket_receive_ += responce->str_ori_;
    std::shared_ptr<WebSocketMessage> msg = std::make_shared<WebSocketMessage>();
    if(!b_market_depth_connected){
      //链接成功后，发送订阅信息
      for(std::pair<std::string, std::vector<std::string>> item1 : market_pair_map_){
        for(std::string item2:item1.second){
          //std::string cmd = "{\"sub\": \"market.btcusdt.trade.detail\",\"id\": \"id generated by client\"}";
          std::string cmd = (boost::format("{\"sub\": \"market.%2%%1%.depth.step0\",\"id\": \"id generated by client\"}")
                             %boost::algorithm::to_lower_copy((item1.first=="USD")?"USDT":item1.first)
                             %boost::algorithm::to_lower_copy(item2)).str();
          std::string send_msg = WebSocketMessage::Encode(cmd);
          socket->SocketSend(send_msg);
          b_market_depth_connected = true;
        }
      }
    }
    while(msg->Decode(market_depth_websocket_receive_)){
      boost::asio::post(thread_pool_, boost::bind(&MarketHuobi::HandleAMessage, this, socket, msg));
    }
  }else{
    trade_history_websocket_receive_.clear();
...
std::string MarketHuobi::HandleWebSocketMessage(const std::string &str_in){
  ...
  try{
    ...
    if(HandleWebSocketPing(pt, rtn)){
      return rtn;
    }else if(HandleWebSocketTradeHistory(pt, rtn)){
      return rtn;
    }else if(HandleWebSocketMarketDepth(pt, rtn)){
      return rtn;
    }
  }catch(...){
...
bool MarketHuobi::HandleWebSocketTradeHistory(boost::property_tree::ptree &pt, std::string &str_out){
  TradeHistory trade_history;
  try{
    std::string ch = pt.get<std::string>("ch");
    boost::property_tree::ptree child_pt = pt.get_child("tick.data");
    for(boost::property_tree::ptree::value_type value:child_pt){
      //for(boost::property_tree::ptree::value_type value_item: value.second.get_child("data")){
        TradeItem trade_item;
        trade_item.id_ = value.second.get<std::string>("id");
        trade_item.price_ = value.second.get<std::string>("price");
        trade_item.amount_ = value.second.get<std::string>("amount");
        trade_item.direction_ = value.second.get<std::string>("direction");
        trade_item.ts = value.second.get<std::string>("ts");
        trade_history.trade_list_.push_back(trade_item);
      //}
    }
    {
      std::lock_guard<std::mutex> lk(coin_info_mutex_);
      std::string coin_for;
      std::string coin_base;
      for(std::pair<std::string, std::vector<std::string>> iter1: market_pair_map_){
        for(std::string iter2: iter1.second){
          if(!boost::algorithm::find_first(ch, boost::algorithm::to_lower_copy(iter2+iter1.first)).empty()){
            coin_for = boost::algorithm::to_upper_copy(iter2);
            coin_base = boost::algorithm::to_upper_copy(iter1.first);
            break;
          }
        }
      }
      trade_history.active_info_.interval_ = ((boost::posix_time::microsec_clock::local_time()-coin_info_[coin_for+"_"+coin_base].last_trade_history_.active_info_.last_time_)).total_milliseconds();
      trade_history.active_info_.last_time_ = boost::posix_time::microsec_clock::local_time();
      coin_info_[coin_for+"_"+coin_base].last_trade_history_.active_info_ = trade_history.active_info_;
      std::list<TradeItem>& trade_list = coin_info_[coin_for+"_"+coin_base].last_trade_history_.trade_list_;
      for(TradeItem& item:trade_history.trade_list_){
        trade_list.push_back(item);
      }
      while(trade_list.size() > 2000){
        trade_list.pop_front();
      }
      DoTradeHistroyChangeSig(coin_for, coin_base, coin_info_[coin_for+"_"+coin_base].last_trade_history_);
      return true;
    }
  }catch(...){//json 解析失败
    //std::cout<<"Huobi trade history info analyze error:"<<std::endl;
  }
  return false;
}

bool MarketHuobi::HandleWebSocketMarketDepth(boost::property_tree::ptree &pt, std::string &str_out){
  DepthInfo depth_info;
  try{
    std::string ch = pt.get<std::string>("ch");
    boost::property_tree::ptree child_bids = pt.get_child("tick.bids");
    ...
    }
    {
      ...
      depth_info.active_info_.interval_ = ((boost::posix_time::microsec_clock::local_time()-coin_info_[coin_for+"_"+coin_base].last_trade_history_.active_info_.last_time_)).total_milliseconds();
      depth_info.active_info_.last_time_ = boost::posix_time::microsec_clock::local_time();
      coin_info_[coin_for+"_"+coin_base].last_trade_history_.active_info_ = depth_info.active_info_;
      coin_info_[coin_for+"_"+coin_base].depth_info_ = depth_info;
      /*std::list<TradeItem>& trade_list = coin_info_[coin_for+"_"+coin_base].last_trade_history_.trade_list_;
      for(TradeItem& item:trade_history.trade_list_){
        trade_list.push_back(item);
      ...
      DoDepthChangeSig(coin_for, coin_base, coin_info_[coin_for+"_"+coin_base].depth_info_);
      return true;
    }
  }catch(...){//json 解析失败
    //std::cout<<"Huobi trade history info analyze error:"<<std::endl;
...
void MarketHuobi::HandleAMessage(std::shared_ptr<AsioHttpsSocket> socket, std::shared_ptr<WebSocketMessage> msg)
{
  ...
  //std::cout<<"str_unzip:"<<str_unzip<<std::endl;
  std::string out = HandleWebSocketMessage(str_unzip);
  if(out.size()){
    std::cout<<"send:"<<out<<std::endl;
    std::string send_msg = WebSocketMessage::Encode(out);
    socket->SocketSend(send_msg);
  }
}

MarketHuobi::MarketHuobi():asio_https_(4),thread_pool_(4){
  InitMarketPair();
  websocket_history_transaction_ = asio_https_.CreateAsioHttpSocket();
  websocket_market_transaction_ = asio_https_.CreateAsioHttpSocket();
}
